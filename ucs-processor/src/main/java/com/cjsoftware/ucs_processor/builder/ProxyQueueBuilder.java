...
import com.cjsoftware.ucs_library.ucs.CachedMethodCall;
...
public class ProxyQueueBuilder extends AbstractClassBuilder<TypeElement> {
      ...
      if (interfaceElement.getKind() == ElementKind.METHOD) {
        ...
        if (!methodElement.getModifiers().contains(Modifier.FINAL)) {
          if (TypeKind.VOID == methodElement.getReturnType().getKind()) {
            ...
            addImplementation(classBuilder, (ExecutableElement) interfaceElement, cacheClass, methodId);
            methodId++;
  ...
  /**
   * Get all members of the supplied type *excluding* members from java.lang (inherited from object)
   *
  ...
  private TypeSpec buildCacheClass(TypeElement proxiedInterface, ExecutableElement interfaceMethod, int methodId) {
    ...
    for (VariableElement methodParam : interfaceMethod.getParameters()) {
      ...
      methodCallCacheClass.addField(TypeName.get(methodParam.asType()), methodParam.getSimpleName().toString(), Modifier.PRIVATE, Modifier.FINAL);
      constructorBuilder.addParameter(TypeName.get(methodParam.asType()), methodParam.getSimpleName().toString());
  ...
  private void addImplementation(TypeSpec.Builder classBuilder, ExecutableElement interfaceMethod, TypeSpec cacheClass, int methodId) {
    ...
    MethodSpec.Builder interfaceProxyMethod = MethodSpec.overriding(interfaceMethod)
        .addModifiers(Modifier.PUBLIC)
